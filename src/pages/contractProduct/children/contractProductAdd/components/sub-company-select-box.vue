<template>
    <div class="s-wrapper">
        <div class="select-show"
             :class="{'active':isShow,'error':error}"
             :style="{width:this.w+'px'}">

            <span class="select-show-txt" v-if="selectText">{{selectText}}</span>
            <span class="select-show-txt default-text"
                  v-else>{{defaultText}}</span>
            <i class="icon layout-center-y icon-arrow-down"></i>
        </div>

        <div class="option-mask"
             :style="{minWidth:this.w+'px'}"
             :class="{opMask:optionWhatStatus}">

            <div class="option-item"
                 v-for="(option,index) in options"
                 @click.stop="selectItem(option)">

                {{option.dictionary_desc}}
            </div>
...
<script type="es6">
    ...
    export default {
        name: 'Select',
        props: ['selectTitle', 'selectBoxName', 'w','defaultTitle', 'options', 'error','dictionary_desc', 'dictionary_code'],
        data ()
        {
            return {
                style: {
        ...
        methods: {
            ...
            selectItem(option){
                ...
                this.bus.$emit('subCompanySelectBoxBus', {
